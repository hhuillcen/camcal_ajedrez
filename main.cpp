         ...
         )
{
    ...
    for( i = 0; i < (int)objectPoints.size(); i++ )
    {
		...
		//colinearidad entre 0 y 6
		for (int x=1; x<=5;x++)
		{
			distancia= distancia_punto_recta(imagePoints2[0],imagePoints2[6],imagePoints2[x]);
			total_distancias=total_distancias+distancia;
			total_puntos_computados=total_puntos_computados+1;
		}

		//colinearidad entre 7 y 13
		for (int x=8; x<=12;x++)
		{
			distancia= distancia_punto_recta(imagePoints2[7],imagePoints2[13],imagePoints2[x]);
			total_distancias=total_distancias+distancia;
			total_puntos_computados=total_puntos_computados+1;
		}

		//colinearidad entre 14 y 20
		for (int x=15; x<=19;x++)
		{
			distancia= distancia_punto_recta(imagePoints2[14],imagePoints2[20],imagePoints2[x]);
			total_distancias=total_distancias+distancia;
			total_puntos_computados=total_puntos_computados+1;
		}

		//colinearidad entre 21 y 27
		for (int x=22; x<=26;x++)
		{
			distancia= distancia_punto_recta(imagePoints2[21],imagePoints2[27],imagePoints2[x]);
			total_distancias=total_distancias+distancia;
			total_puntos_computados=total_puntos_computados+1;
		}

		//colinearidad entre 28 y 34
		for (int x=29; x<=33;x++)
		{
			distancia= distancia_punto_recta(imagePoints2[28],imagePoints2[34],imagePoints2[x]);
			total_distancias=total_distancias+distancia;
			total_puntos_computados=total_puntos_computados+1;
		}
    }
...
//  vector<Vec4i> hierarchy;
  vector<Point2f> puntos;
	...
	//deteccion = findCirclesGrid(src_gray, patternsize, puntos, CALIB_CB_SYMMETRIC_GRID | CALIB_CB_CLUSTERING, blobDetector);
...
int main( int argc, char** argv )
{
 ...
 //  intrinsic.ptr<float>(1)[1] = 1;
	...
	VideoCapture vc("d://casa//Chessboard1.mp4");
	...
	while (true)
	{
		...
		if(cont_imag==nro_imagenes)
		{
			...
			cout<<endl<<"EMPEZANDO CALIBRACION CON  :  "<< cont_imag <<"  imagenes correctamente detectadas"<<endl;
			  ...
			  calibrateCamera(coord3D, coord2D,src.size(), intrinsic, distCoeffs, rvecs, tvecs);//,CV_CALIB_USE_INTRINSIC_GUESS | CV_CALIB_RATIONAL_MODEL);
		...
		if (cont_imag>nro_imagenes)
		{
				VideoCapture vc1("d://casa//Chessboard1.mp4");
